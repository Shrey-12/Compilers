#include <iostream>
#include <string.h>
#include <fstream>
#include <map>
#include <vector>
#define max 20

using namespace std;

char productions[max][10];
char Terminals[10], nonTerminals[10];
char first[10][10], follow[10][10];
int eps[10];
int count = 0;
map<char, vector<string>> production_map;

// find position in production
int findpos(char ch)
{
    int n;
    for (n = 0; nonTerminals[n] != '\0'; n++)
        if (nonTerminals[n] == ch)
            break;
    if (nonTerminals[n] == '\0')
        return 1;
    return n;
}

int IsCap(char c)
{
    if (c >= 'A' && c <= 'Z')
        return 1;
    return 0;
}
// if not already present in array add
void add(char *arr, char c)
{
    int i, flag = 0;
    for (i = 0; arr[i] != '\0'; i++)
    {
        if (arr[i] == c)
        {
            flag = 1;
            break;
        }
    }
    if (flag != 1)
        arr[strlen(arr)] = c;
}

// adding s2 to s1, excluding whats already present
void addarr(char *s1, char *s2)
{
    for (int i = 0; s2[i] != '\0'; i++)
    {
        int flag = 0;
        int j = 0;
        while (true)
        {
            if (s2[i] == s1[j])
            {
                flag = 1;
                break;
            }
            if (j == strlen(s1) && flag != 1)
            {
                s1[strlen(s1)] = s2[i];
                break;
            }
            j++;
        }
    }
}

// to add production to productions array
void addprod(char *s)
{
    int i;
    productions[count][0] = s[0];

    for (i = 3; s[i] != '\0'; i++)
    {
        if (!IsCap(s[i]))
            add(Terminals, s[i]);
        productions[count][i - 2] = s[i];
    }
    productions[count][i - 2] = '\0';
    char nonTerminal = s[0];
    string productionString = productions[count];

    if (production_map.find(nonTerminal) == production_map.end())
    {
        // If non-terminal not found in the map, add it
        vector<string> productionsVector;
        productionsVector.push_back(productionString);
        production_map[nonTerminal] = productionsVector;
    }
    else
    {
        // If non-terminal found, append the production to the existing vector
        production_map[nonTerminal].push_back(productionString);
    }
    add(nonTerminals, s[0]);
    count++;
}

void findfirst()
{
    int e;
    // looping through productions
    for (int i = 0; i < count; i++)
    {
        // iterate over each production again
        for (int j = 0; j < count; j++)
        {
            int n = findpos(productions[j][0]);
            if (productions[j][1] == '#')
                eps[n] = 1;
            else
            {
                // process each symbol in production
                for (int k = 1, e = 1; productions[j][k] != '\0' && e == 1; k++)
                {
                    // if symbol is terminal add it to first
                    if (!IsCap(productions[j][k]))
                    {
                        e = 0;
                        add(first[n], productions[j][k]);
                    }
                    else
                    {
                        int n1 = findpos(productions[j][k]);
                        addarr(first[n], first[n1]);
                        // if eps flag is not set for non terminal set e to 0
                        if (eps[n1] == 0)
                            e = 0;
                    }
                }
                if (e == 1)
                    eps[n] = 1;
            }
        }
    }
}

void findfollow()
{
    int i, j, k, n, e, n1;
    // find position of starting non terminal and add $ to its follow
    n = findpos(productions[0][0]);
    add(follow[n], '$');
    for (i = 0; i < count; i++)
    {
        for (j = 0; j < count; j++)
        {
            // initialize k to index of last symbol
            k = strlen(productions[j]) - 1;
            // iterate through each symbol in reverse order
            for (; k > 0; k--)
            {
                if (IsCap(productions[j][k]))
                {
                    n = findpos(productions[j][k]);
                    // if next symbol is end of production, add follow of left non Terminal
                    if (productions[j][k + 1] == '\0')
                    {
                        n1 = findpos(productions[j][0]);
                        addarr(follow[n], follow[n1]);
                    }
                    // if next symbol is non Terminal add first of next in currents follow
                    if (IsCap(productions[j][k + 1]))
                    {
                        n1 = findpos(productions[j][k + 1]);
                        addarr(follow[n], first[n1]);
                        // if next non Terminal derives eps, add follow of LHS non terminal
                        if (eps[n1] == 1)
                        {
                            n1 = findpos(productions[j][0]);
                            addarr(follow[n], follow[n1]);
                        }
                    }
                    else if (productions[j][k + 1] != '\0')
                        add(follow[n], productions[j][k + 1]);
                }
            }
        }
    }
}

void findIntersection(const char *s1, const char *s2, char *result)
{
    int resultIndex = 0;

    for (int i = 0; s1[i] != '\0'; i++)
    {
        for (int j = 0; s2[j] != '\0'; j++)
        {
            if (s1[i] == s2[j])
            {
                result[resultIndex++] = s1[i];
                break; // Break to avoid duplicates in the result
            }
        }
    }

    result[resultIndex] = '\0';
}
bool isLL1()
{
    // Check for left recursion
    for (int i = 0; i < count; i++)
    {
        if (productions[i][0] == productions[i][1])
        {
            cout << "Left recursion detected." << endl;
            return false;
        }
    }

    for (int i = 0; i < count; i++)
    {
        int n = findpos(productions[i][0]);
        char commonChars[10];
        strcpy(commonChars, first[n]);

        // Check for intersection of first and follow sets

        if (IsCap(productions[i][1]))
        {
            int n1 = findpos(productions[i][1]);
            findIntersection(commonChars, first[n1], commonChars);

            // Check if there are common characters in the intersection
            if (strlen(commonChars) > 0)
            {
                return false;
            }
        }
        // If epsilon belongs to the first set of the current symbol, check intersection between follow set of n and follow set of the left-hand side
        else if (productions[i][1] == '#')
        {
            int n1 = findpos(productions[i][0]);

            // Check if there is an intersection between follow set of n and follow set of n1
            char commonChars[10]; // Assuming a maximum of 10 common characters
            findIntersection(follow[n], follow[n1], commonChars);

            // Check if there are common characters in the intersection
            if (strlen(commonChars) > 0)
            {
                return false;
            }
        }
    }
    return true;
}
int main()
{
    char s[max], i;
    ifstream inputFile("input.txt");
    while (inputFile >> s)
    {
        addprod(s);
    }
    inputFile.close();

    findfirst();
    findfollow();
    for (i = 0; i < strlen(nonTerminals); i++)
    {
        cout << nonTerminals[i] << "\t";
        cout << first[i];
        if (eps[i] == 1)
            cout << '#' << "\t";
        else
            cout << "\t";
        cout << follow[i] << endl;
    }
    cout << endl;
    cout << "Productions:\n";
    for (int i = 0; i < count; i++)
    {
        cout << productions[i] << endl;
    }

    cout << endl;
    cout << "First:\n";
    for (int i = 0; i < count; i++)
    {
        cout << first[i] << endl;
    }

    cout << "Follow:\n";
    for (int i = 0; i < count; i++)
    {
        cout << follow[i] << endl;
    }
    if (isLL1())
    {
        cout << "Grammar is LL(1)" << endl;
    }
    else
    {
        cout << "Grammar is not LL(1)" << endl;
    }
    return 0;
}
